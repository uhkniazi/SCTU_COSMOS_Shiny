# Name: Cosmos_query_app.R
# Auth: u.niazi@soton.ac.uk
# Date: 06/12/2024
# Desc: shiny app for querying the database


source('globals.R')

ui = fluidPage(
  titlePanel("Test: COSMOS Query Module"),
  sidebarLayout(
    sidebarPanel(
      textInput("inSearchTerms", "Enter search query"),
      uiOutput("outMatchingTerms"),
      uiOutput("outFunctionArgs"),
      actionButton("inQueryButton", "Query DB")
    ),
    mainPanel(
